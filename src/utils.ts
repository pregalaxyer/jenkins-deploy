...
export function getValidEditor() {
  let langIds = ["groovy", "jenkinsfile", "java"];
...
export async function withProgressOutputParallel(
  title: string,
  items: any[],
  func: (i: any) => Promise<string>
) {
  return await vscode.window.withProgress(
    {
      location: vscode.ProgressLocation.Window,
    ...
    let conns = [
      {
        name: "default",
        uri:
          undefined === jenkinsConfig.uri
            ? "http://127.0.0.1:8080"
            : jenkinsConfig.uri,
        username:
          undefined === jenkinsConfig.username ? null : jenkinsConfig.username,
        password:
          undefined === jenkinsConfig.password ? null : jenkinsConfig.password,
        active: true,
      },
    ];
...
export function addeNodeLabelsScript(
  nodes: string[],
  labels: string[]
): string {
  let labelsToken = "";
  ...
  return `import jenkins.model.*;
    ...
    jenkins.save();`
    .replace("<<LABELS>>", labelsToken)
    .replace("<<NODES>>", nodesToken);
}

export async function parallelTasks<T>(
  items: any,
  action: (item: any) => Promise<T>
): Promise<T[]> {
  let tasks: Promise<T>[] = [];
...
export function updateNodeLabelsScript(
  nodes: string[],
  labels: string[]
): string {
  let labelsToken = "";
